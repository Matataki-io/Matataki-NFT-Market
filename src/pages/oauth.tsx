...
import { fetchUserStat, getFullUrlOfFileInCDN } from "../utils/matataki.service"
...
export default function handleOAuth() {
    ...
    async function tryToFetchUserData(token: string) {
        ...
        if (axios.isAxiosError(error)) {
            if (error.response?.status === 401) updateError("Server says it is a bad token")
        }
    ...
    if (errorMsg) return <div className="oauth-error">
        <Text h2>Sorry</Text>
        <Text>There's problem when connecting to Matataki. Reason: <code>{errorMsg}</code>. Please contact the team asap.</Text>
    </div>
    if (userProfile) return <div className="oauth-success" style={{ textAlign: 'center' }}>
        <Text h2>Welcome back</Text>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
            You are logging as
            <User src={getFullUrlOfFileInCDN(userProfile.avatar)} name={userProfile.nickname || userProfile.username} />
        </div>
    </div>
    return <div className="handleOAuth">
